...
import {
  buildBrainMapNodes,
  DEFAULT_BRAIN_MAP_CONFIG,
  type BrainMapConfig,
} from "@/app/lib/app/brain-map-config";

type CreativeBrainMapProps = {
  config?: BrainMapConfig;
};

function BrainMapLegend({ nodes }: { nodes: ReturnType<typeof buildBrainMapNodes> }) {
  return (
...
function BrainMapSvg({ nodes }: { nodes: ReturnType<typeof buildBrainMapNodes> }) {
  return (
...
export function CreativeBrainMap({
  config = DEFAULT_BRAIN_MAP_CONFIG,
}: CreativeBrainMapProps) {
  const nodes = buildBrainMapNodes([...MOCK_UNIVERSES]);

  if (config.mode === "image" && config.imageSrc) {
    return (
      <div className="creative-brain-map creative-brain-map--image" aria-label="Mapa mental dos universos">
        <div className="creative-brain-map__asset">
          <Image
            src={config.imageSrc}
            alt={config.imageAlt ?? "Constelação criativa"}
            width={640}
            height={560}
            className="creative-brain-map__image"
            sizes="(max-width: 768px) 100vw, 16rem"
          />
          <div className="creative-brain-map__asset-overlay" aria-hidden />
        </div>
        <BrainMapLegend nodes={nodes} />
      </div>
    );
  }

  return (
    <div className="creative-brain-map creative-brain-map--svg" aria-label="Mapa mental dos universos">
      <BrainMapSvg nodes={nodes} />
      <BrainMapLegend nodes={nodes} />
    </div>
  );
}
