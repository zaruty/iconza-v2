// @ts-nocheck — páginas internas: tipagem na fase seguinte
"use client";

import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { DashboardLayout } from "./dashboard-layout";
import { NOTIFICACOES } from "./dashboard-nav-data";
import { T, c1, c2, c3, cd, cb } from "./dashboard-tokens";
import { Icon3D } from "./dashboard-icons";
import type { PageId } from "./dashboard-types";
...
export default function DashboardPage() {
  ...
  const render = () => {
    switch (page) {
      case "inicio":
        return <PageInicio isDark={isDark} notifs={notifs} onAbrirNotif={() => setNotifOpen((o) => !o)} />;
      case "universos":
        return <PageUniversos isDark={isDark} />;
      case "cerebro":
        return <PageCerebro isDark={isDark} />;
      case "iconplay":
        return <PageIconplay isDark={isDark} />;
      case "apoio":
        return <PageApoio isDark={isDark} />;
      case "perfil":
        return <PagePerfil isDark={isDark} />;
      case "configuracoes":
        return <PageConfiguracoes isDark={isDark} setIsDark={setIsDark} />;
      case "ia":
        return (
          <PageEmConstrucao
            isDark={isDark}
            ...
            iconKey="ia"
            color="#C26D8C"
            descricao="O centro de inteligência do ecossistema ICONZA. Um mentor virtual que aprende com o seu comportamento e cria trilhas personalizadas."
          />
        );
      case "cursos":
        return (
          <PageEmConstrucao
            ...
            iconKey="cursos"
            color="#52B87A"
            descricao="Biblioteca completa de conteúdos dos universos — videoaulas, podcasts, ebooks e workshops ao vivo."
          />
        );
      case "colecoes":
        return (
          <PageEmConstrucao
            ...
            iconKey="colecoes"
            color="#6B8FC7"
            descricao="O seu espaço de organização pessoal. Salve aulas, ferramentas, referências e crie coleções temáticas."
          />
        );
      case "marketplace":
        return (
          <PageEmConstrucao
            ...
            iconKey="marketplace"
            color="#D97832"
            descricao="A economia criativa do ICONZA. Cursos premium, mentorias, templates e serviços dos melhores criadores."
          />
        );
      default:
        return <PageInicio isDark={isDark} notifs={notifs} onAbrirNotif={() => setNotifOpen((o) => !o)} />;
